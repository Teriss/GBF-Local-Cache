...
const GITHUB_ISSUE_URL = 'https://github.com/Teriss/GBF-Local-Cache/issues/new'
  ...
  return (
    <div className="page-grid overview-grid">
      ...
      <section className="split-grid">
        <div className="panel"><div className="panel-heading"><div><span className="section-label">当前网络模式</span><h3>{snapshot.network_mode === 'direct' ? '系统直连' : 'Clash'}</h3></div></div><p className="muted">代理监听 <code>{snapshot.listen_address}</code>，只绑定本机回环地址。</p></div>
...
function NetworkPage({ snapshot, backend, onNotify }: { snapshot: Snapshot; backend?: BackendApp; onNotify: Notify }) {
  ...
  const applyMode = async (nextMode: 'direct' | 'clash') => {
    setMode(nextMode)
    if (nextMode === 'direct') {
      onNotify('已选择系统直连；是否经过 UU 取决于 UU 对本程序的加速规则。')
    } else {
